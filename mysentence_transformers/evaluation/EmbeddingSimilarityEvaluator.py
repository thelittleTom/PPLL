...
class EmbeddingSimilarityEvaluator(SentenceEvaluator):
    ...
    def eval(self, model_embed, eval_add_label=True):
        ...
        #     print(f'{k} : {np.mean(v)}')
        return np.mean(all_sss), np.mean(all_score), np.mean(all_acc), np.mean(all_NMI)
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1):
        ...
        print('without mini-cluster labels')
        sss, score, acc, nmi = self.eval(model, eval_add_label=False)
        print('with mini-cluster labels')
        sss2, score2, acc2, nmi2 = self.eval(model, eval_add_label=True)
        if sss >= sss2:
            return {'sss': sss, 'score': score, 'mini_cluster_labels': False, 'acc': acc, 'nmi': nmi}
        else:
            return {'sss': sss2, 'score': score2, 'mini_cluster_labels': True, 'acc': acc2, 'nmi': nmi2}
