...
# loss fn
def loss_fn(x, y,y_,l,if_neg=False,mode=0,mode_params=None):
	temperature =1
...
class BYOLoss5(nn.Module):
	...
	def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor,device,mode_params):
		...
		loss_one = loss_fn(online_pred_one, target_proj_two.detach(),target_proj_one.detach(),labels,self.if_neg,self.mode,mode_params)
		loss_two = loss_fn(online_pred_two, target_proj_one.detach(),target_proj_two.detach(),labels,self.if_neg,self.mode,mode_params)
